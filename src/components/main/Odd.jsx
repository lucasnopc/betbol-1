        ...
        <div className={`text-${orientation} h-full border-gray-200 divide-x-2 py-1 mr-2 divide-white`}>
            {values && values.length <= 3 && <>
                ...
                {nVals && <div className="inline-block">
                    <Link href={`/fix/${id}`} ><a className={`rounded-lg w-${squadWidth} h-${squadWidth} text-primary cursor-pointer active:outline-none focus:outline-none flex items-center justify-center min-w-full bg-gray-200 hover:bg-gray-300 text-xs font-normal`}>+{nVals}</a></Link>
                </div>}
            </>
            }
