...
export default function AllPays(props) {
    ...
    const receiverPoints = async (pay) => {
    ...
    return <>
        {payments.reverse().map(pay => {
            ...
            return <div className="bg-gray-100 p-2 shadow-sm my-1 grid grid-cols-4" key={pay.id}>
                ...
                <div> {status(pay.values) && !pay.received && <button onClick={() => receiverPoints(pay)} className="float-right uppercase font-semibold text-sm text-white bg-green-500 hover:bg-green-400 p-1">Resgatar Pontos</button>
                }
