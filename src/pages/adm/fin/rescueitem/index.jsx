...
import Bilhete from '../../../../components/bilhete/index'
...
export default function RescueItem({ r }) {
  ...
  const openPayment = async () => {
    setLoading(true)
    const user = await axios.get(`/api/adm/getusers?email=${r.email}`)
    setUser(user.data.accounts[0])
    setLoading(false)
  }
  const onSubmit = async data => {
    const body = {_id: r._id, code: data.code, state: "payment_made"}
  ...
  return <><div className="bg-gray-100 flex divide-x-2 justify-between text-gray-700 p-1 m-0.5 shadow-md" key={r._id}>
    <div>
      <span  onClick={() => { setToggleTicket(true) }} className="hover:text-primary cursor-pointer"><span className="font-semibold">bilhete:</span> {r._id}</span><br />
    ...
    <div>
      ...
      <span className="font-semibold">Método: </span>{r.status.method}
    </div>
      ...
      {user && <div className="absolute bg-black bg-opacity-70 flex items-center justify-center top-0 left-0 w-full h-full">
            <div className="relative bg-white shadow-md rounded-md p-2">
               {user.payment_method && <><span onClick={()=>{setUser(false)}} className="text-red-300 hover:text-red-500 cursor-pointer font-semibold absolute right-0 -top-5">[x] fechar</span>
                <p>Faça o pagamento de R$ {Number(r.potencialReturn).toFixed(2)} para o pix {user.payment_method.chave} de chave <span className="semibold">{user.payment_method.pix}</span></p>
                <p>Insira o comprovante abaixo</p>
                <form onSubmit={handleSubmit(onSubmit)}>
                  <input {...register("code", { required: true })} type="text" placeholder="Nº comprovante da transação!" />
                  <input type="submit" value="Clique aqui após fazer o pagamento pix" className="bg-primary text-white mx-1 cursor-pointer hover:bg-primary-ligth font-semibold p-1" />
                </form></>}
